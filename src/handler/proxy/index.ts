...
async function wrapUpstreamResponse(upstream: Response): Promise<Response> {
    ...
    console.log("[Proxy] wrapUpstreamResponse - Content-Encoding:", contentEncoding, "Status:", upstream.status);

    // Read the body content
    let body = await upstream.arrayBuffer();
    console.log("[Proxy] Body size before decompression:", body.byteLength);

    // Decompress gzip/deflate if needed (Node.js fetch doesn't auto-decompress)
    if (contentEncoding === "gzip" || contentEncoding === "deflate" || contentEncoding === "br") {
        console.log("[Proxy] Attempting decompression for encoding:", contentEncoding);
        try {
            ...
            body = decompressed.buffer.slice(decompressed.byteOffset, decompressed.byteOffset + decompressed.byteLength) as ArrayBuffer;
            console.log("[Proxy] Decompression successful, new size:", body.byteLength);
        } catch (e) {
            // If decompression fails, return original body (might already be decompressed)
            console.warn("[Proxy] Decompression failed, using original body:", (e as Error)?.message);
        }
    }

    // Remove encoding headers since we've decompressed
...
export function withProxy(targetUrl: string, hooks: Hook[]) {
    return async (req: Request): Promise<Response> => {
        ...
        if (isNotification) {
            ...
            // Node.js fetch doesn't auto-decompress, which breaks JSON parsing
